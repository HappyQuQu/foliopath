...
func TestComposeOwnsValidatedConfiguration(t *testing.T) {
	...
	if got, want := strings.Join(componentNames, ","), "database,system-events,resource-profile,media-root,image-runtime,media-worker,scan-worker,automatic-discovery,library-removal-worker,http,readiness"; got != want {
