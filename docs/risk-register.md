...
| R-005 | 10 万媒体/1 万目录目标超出四核、4 GiB 环境的扫描、查询或内存能力 | 高 | 高 | 队列持续增长、首次扫描不可接受、列表延迟或 RSS 超预算 | [FS-04](spikes/fs-04-capacity-baseline.md) 与 [S2-106](gates/MVP-2026-07-23/s2-scan-capacity.md)已覆盖扫描主档；[S4-003](gates/MVP-2026-07-23/s4-search-backend-ready.md)又在 macOS arm64 四核与 Linux arm64 2 CPU/4 GiB 的 100k/10k 档覆盖扫描并发搜索、FTS/短词/全局/keyset、取消、rebuild、RSS/heap/DB，均无预算超限。Linux amd64/arm64 同一强制档已接入 CI；HTTP/前端、媒体组合和代表性存储继续按 S0-106 分配到后续 Gate | 降低并发/缩略图规格，声明实测支持上限；架构变更须 ADR | 技术负责人 | 缓解中 |
