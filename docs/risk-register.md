...
| R-010 | 认证未完成的预览版或失效的稳定版认证被暴露到不可信网络 | 高 | 严重 | Compose 改为全网绑定、代理绕过、会话/CSRF 缺陷或公开扫描发现端口 | 预览版默认回环；[S1-101 Contract Ready](gates/MVP-2026-07-23/s1-auth-contract-ready.md) 已固定单管理员、Origin、Cookie/CSRF、防缓存、稳定错误码和摘要存储；S1-102～106 仍须实现会话、退出、代理信任、限流和安全测试 | 立即撤销公网暴露，只允许本机/可信隧道访问 | 安全负责人 | 缓解中 |
