...
| R-006 | 畸形媒体触发 libvips/FFmpeg 崩溃或资源耗尽 | 中 | 严重 | 解析超时、OOM、子进程堆积或相同文件无限重试 | [FS-03](spikes/fs-03-media-matrix.md)、[S3-004](gates/MVP-2026-07-23/s3-media-processing.md) 与 [S3-005](gates/MVP-2026-07-23/s3-media-jobs-cache.md) 已验证 production adapter 的截断拒绝、稳定错误、FFmpeg 超时/取消、初始上限、2-worker/3-attempt durable 隔离和退避；像素炸弹、原生调用不可中断区间及资源峰值仍由 S3-006 阻断 | 隔离失败资产；临时禁用受影响格式或视频封面 | 媒体处理负责人 | 缓解中 |
...
| R-009 | 10 GiB 缩略图缓存、临时文件或 WAL 仍耗尽磁盘 | 中 | 高 | 可用空间持续下降、LRU 无法维持余量、临时文件残留或写事务失败 | [S3-005](gates/MVP-2026-07-23/s3-media-jobs-cache.md) 已实现可配置 10 GiB 默认配额、90%→80% LRU、512 MiB 发布余量、原子写入、启动/定期/设置唤醒清理和 fingerprint 失效文件队列；真实磁盘满、WAL/temp 竞争与容量压力仍由 S3-006/Release Gate 验证 | 暂停派生任务并清理可重建缓存，优先保护数据库 | 运维负责人 | 缓解中 |
...
| R-013 | 多媒体库共享任务队列时出现饥饿或请求被后台工作拖慢 | 中 | 高 | 一个大库占满 worker；API 延迟随扫描显著上升 | [S2-102](gates/MVP-2026-07-23/s2-bounded-scan-worker.md) 与 [S2-106](gates/MVP-2026-07-23/s2-scan-capacity.md) 已固定扫描 2-worker 公平队列；[S3-005](gates/MVP-2026-07-23/s3-media-jobs-cache.md) 复用同一 generic lease worker，并以 durable library fairness cursor 固定媒体 2-worker/3-attempt 跨库领取。媒体资源峰值与正式 HTTP catalog 延迟仍由 S3-006/007 验证 | 暂停/限速后台任务，改为手动扫描模式 | 技术负责人 | 缓解中 |
...
风险，但不等于发布风险关闭。
R-009 已有 S3-005 实现证据而转为“缓解中”，但磁盘满、WAL/temp 竞争和发布存储压力仍
未关闭。不得把产品确认或局部实现误写成完整技术可行性证明。
