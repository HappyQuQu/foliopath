...
| R-016 | 取消自动 CI 后漏跑本地检查或干净环境发生漂移 | 高 | 高 | 合并前未记录本地验证；生成文件与源不一致；目标架构发布失败 | `Makefile` 保留格式、架构、生成、lint、单元、集成与 E2E 统一入口；开发者在合并和发布前本地执行并记录结果，Docker Hub workflow 只负责 Release/手动双架构构建；历史双架构、mount、媒体、runtime/recovery 与供应链证据继续保留 | 阻止合并和发布，先在干净工作树恢复并完成本地验证基线 | QA 负责人 | 缓解中 |
...
| R-025 | 模型、权重或向量引擎许可/供应链不允许产品分发或引入高危 native 闭包 | 高 | 严重 | 权重只有研究/非商用授权；来源/hash 不清；双架构 SBOM 出现未处置发现 | 模型 manifest、哈希、独立权重许可审查、原生双架构 SBOM/provenance；ADR-0013 接受前不得选型 | 更换许可清晰模型；无法替换则删除对应能力 | 合规、安全与发布负责人 | 开放 |
