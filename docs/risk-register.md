...
| R-010 | 认证未完成的预览版或失效的稳定版认证被暴露到不可信网络 | 高 | 严重 | Compose 改为全网绑定、代理绕过、会话/CSRF 缺陷或公开扫描发现端口 | 预览版默认回环；S1-101～103 已固定契约并实现密码、原子 setup、高熵摘要会话、绝对期限与退出撤销；S1-104～106 仍须实现 CSRF/default-deny、代理信任、限流、handler 和安全测试 | 立即撤销公网暴露，只允许本机/可信隧道访问 | 安全负责人 | 缓解中 |
