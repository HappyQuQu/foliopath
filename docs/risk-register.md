...
| R-005 | 10 万媒体/1 万目录目标超出四核、4 GiB 环境的扫描、查询或内存能力 | 高 | 高 | 队列持续增长、首次扫描不可接受、列表延迟或 RSS 超预算 | 历史 [S5-005](gates/MVP-2026-07-23/s5-release-capacity-candidate.md) 证据保留；2026-08-31 当前强制基线的 production keyset P95 296.212 ms 再次超过 250 ms，已由[容量回归复审](gates/MVP-2026-07-23/s4-search-capacity-regression-2026-08-31.md)失败关闭。order-first 候选在两库 100k 正确性矩阵通过，但尚未获 production/native 双架构接受 | 保留现有生产 SQL并阻断最终容量 Gate；补齐候选完整矩阵和双架构后再决定，禁止提高预算或降低搜索语义 | 技术负责人 | 重新开放 |
