...
| R-010 | 认证未完成的预览版或失效的稳定版认证被暴露到不可信网络 | 高 | 严重 | Compose 改为全网绑定、代理绕过、会话/CSRF 缺陷或公开扫描发现端口 | 预览版默认回环；S1-101 已固定认证契约，S1-102 已实现 Argon2id、身份规范化和原子单管理员 setup；S1-103～106 仍须实现安全会话、退出、CSRF、代理信任、限流、handler 和安全测试 | 立即撤销公网暴露，只允许本机/可信隧道访问 | 安全负责人 | 缓解中 |
