...
| R-010 | 认证未完成的预览版或失效的稳定版认证被暴露到不可信网络 | 高 | 严重 | Compose 改为全网绑定、代理绕过、会话/CSRF 缺陷或公开扫描发现端口 | 预览版默认回环；S1-101～105 已实现冻结契约、密码/原子 setup、高熵摘要会话、绝对期限/撤销、真实 handler、Origin/CSRF、默认拒绝、防缓存、直连 peer 限流，以及错误/并发/时间安全矩阵；S1-106 仍须完成 Backend Ready，可信代理与网络发布由 Stage 5 验收 | 立即撤销公网暴露，只允许本机/可信隧道访问 | 安全负责人 | 缓解中 |
