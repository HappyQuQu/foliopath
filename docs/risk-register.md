...
| R-005 | 10 万媒体/1 万目录目标超出四核、4 GiB 环境的扫描、查询或内存能力 | 高 | 高 | 队列持续增长、首次扫描不可接受、列表延迟或 RSS 超预算 | [FS-04](spikes/fs-04-capacity-baseline.md) 与 [S2-106](gates/MVP-2026-07-23/s2-scan-capacity.md) 已覆盖混合深度/宽度主档、1,000 层 finalize、并发索引读、RSS/heap/DB 预算和三档趋势；正式 256 batch 的 10k/100k 本地档无预算超限，Linux amd64/arm64 独立容量 job 已接入 CI。[S4-002](gates/MVP-2026-07-23/s4-search-keyset.md)已建立 trigram FTS + 精确短词 fallback/keyset 实现，但其 100k 搜索与扫描并发预算仍由 S4-003 阻断。HTTP/前端和代表性存储继续按 S0-106 分配到后续 Gate | 降低并发/缩略图规格，声明实测支持上限；架构变更须 ADR | 技术负责人 | 缓解中 |
...
| R-009 | 10 GiB 缩略图缓存、临时文件或 WAL 仍耗尽磁盘 | 中 | 高 | 可用空间持续下降、LRU 无法维持余量、临时文件残留或写事务失败 | [S3-005](gates/MVP-2026-07-23/s3-media-jobs-cache.md) 已实现 10 GiB/90%→80% LRU/512 MiB 余量；[S3-006](gates/MVP-2026-07-23/s3-media-resource-safety.md) 以真实 8 MiB tmpfs `ENOSPC` 验证临时文件清理、DB 不误提交、退避和释放空间后恢复。长期 WAL/temp 竞争、卷 sizing 与代表性存储仍由 Release Gate 验证 | 暂停派生任务并清理可重建缓存，优先保护数据库 | 运维负责人 | 缓解中 |
...
| R-012 | Unicode、大小写和文件系统差异造成重复路径或搜索错误 | 中 | 高 | macOS/Linux 结果不同、唯一约束冲突、不可定位文件 | S2-007 已确认媒体库显示名 NFC、唯一键 NFKC + Unicode full case folding、locale-neutral numeric sort key 与组件级根比较；[S4-002 搜索实现](gates/MVP-2026-07-23/s4-search-keyset.md)已用组合字符、sharp-s、全角、中文、保留变音符号、短词和 `%/_` 标点 fixture 验证 FTS 候选与精确字面 AND 路径一致。真实跨文件系统和扩展容量矩阵仍由 S4-003/Release Gate 验证 | 保留规范显示名并拒绝歧义库，搜索实现不满足 profile 时停止 Backend Gate；必要时缩小受支持文件系统范围 | 后端负责人 | 缓解中 |
| R-013 | 多媒体库共享任务队列时出现饥饿或请求被后台工作拖慢 | 中 | 高 | 一个大库占满 worker；API 延迟随扫描显著上升 | [S2-102](gates/MVP-2026-07-23/s2-bounded-scan-worker.md) 与 [S2-106](gates/MVP-2026-07-23/s2-scan-capacity.md) 固定扫描 2-worker 公平队列；[S3-005](gates/MVP-2026-07-23/s3-media-jobs-cache.md) 固定媒体 2-worker/3-attempt 跨库公平，[S3-006](gates/MVP-2026-07-23/s3-media-resource-safety.md) 固定 govips concurrency 1 和 FFmpeg 单 decoder/filter thread；[S3-007](gates/MVP-2026-07-23/s3-browse-thumbnail-backend-ready.md) 证明 HTTP 只读索引/派生缓存，缺失缓存只重排任务、不在请求线程处理。代表性设备长期并发仍由 Release Gate 验证 | 暂停/限速后台任务，改为手动扫描模式 | 技术负责人 | 缓解中 |
...
风险，但不等于发布风险关闭。
R-009 已有 S3-005 配额实现和 S3-006 真实 tmpfs 满盘证据，但长期 WAL/temp 竞争和发布
存储压力仍未关闭。不得把产品确认或局部实现误写成完整技术可行性证明。
