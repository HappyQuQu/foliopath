...
**Accepted for fail-closed backend implementation（2026-09-01）**。产品已授权 POST-MVP-5 revision 2 的
全部 S2 后端继续实现，同时要求发布保持 fail-closed。本 ADR 因而接受方案 3 的 tokenizer/runtime、format v2
与 activation 边界，允许在 production source 中实现严格 parser、FD-anchored adapter 和 text-session owner；
它不批准候选模型字节、不允许填写内建 catalog，也不允许在最终质量、原生双架构、供应链和安全签署前把
SentencePiece 链入发行镜像或启用消费者 UI。

2026-08-29 的历史逐门槛判断见
[ADR-0014 接受审计](../gates/POST-MVP-5/adr-0014-acceptance-audit-2026-08-29.md)：
仅特殊 token 失败关闭合同已通过，其余门槛仍为 Partial/Blocked。接受 fail-closed 后端架构不改变这些发布
门槛；不得把 QEMU amd64、incomplete SBOM 或动态 tripwire 提升为 native、final signed provenance 或 VEX。
...
final-image 漏洞处置、final signed SBOM/provenance 和负责人签署仍缺，故这些证据不足以批准发行镜像
链接 SentencePiece。production fail-closed composition 已完成，但 release image linking 仍禁止。隔离合并器已为当前 arm64 与 QEMU amd64 镜像生成
