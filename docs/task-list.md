...
一句话：**认证、媒体库管理和可靠扫描后端已经 Backend Ready；浏览目录树/详情和媒体
处理基础已完成，后端下一项是 `S3-005` 有界媒体任务与缓存保护，前端可以接入真实扫描 API。**
...
| 1F. 前端基础与登录 | React 应用壳、设计系统和认证页面 | 🟡 独立前端任务 | 可以初始化管理员并登录 |
| 2. 媒体库和扫描 | 在设置中选择 `/library` 子目录，创建多个媒体库并扫描 | ✅ 后端已完成，前端待集成 | 可以创建媒体库并看到扫描状态 |
| 3. 文件夹浏览和缩略图 | 展示目录树、图片/视频缩略图和递归浏览 | 🟡 目录与媒体处理基础完成 | 可以按真实文件夹浏览媒体 |
...
当前后端里程碑：**Stage 3 / 浏览与缩略图**。媒体库 `S2-001～007` 已通过 Backend Ready，
...
`S3-004` 已实现媒体探测、缩略图/视频封面、派生状态和原子缓存发布；当前后端任务是
`S3-005` 有界媒体任务、fingerprint 失效、LRU 与磁盘保护。Stage 4～5 仍未授权。
