...
一句话：**认证、媒体库管理和可靠扫描后端已经 Backend Ready；目录、媒体处理和后台
缓存任务已完成，后端下一项是 `S3-006` 敌意输入与资源矩阵，前端可以接入真实扫描 API。**
...
| 3. 文件夹浏览和缩略图 | 展示目录树、图片/视频缩略图和递归浏览 | 🟡 后台派生与缓存保护完成 | 可以按真实文件夹浏览媒体 |
...
durable 媒体任务、fingerprint 失效、LRU 与磁盘保护。当前后端任务是 `S3-006`
敌意媒体、磁盘满与并发资源矩阵。Stage 4～5 仍未授权。
