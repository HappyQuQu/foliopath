...
一句话：**认证、媒体库、扫描、浏览与缩略图后端已经 Backend Ready；`S4-001` 搜索契约
已冻结，后端下一项是 `S4-002` 搜索实现；前端可以接入真实资产分页、详情和缩略图 API，
但还不能接入搜索。**
...
| 4. 搜索和查看器 | 搜索、筛选、查看大图和播放兼容视频 | 🟡 搜索契约已完成，功能未实现 | MVP 的主要使用流程完整 |
...
`S4-001` 已固定搜索 profile、三种 scope、筛选/排序与 cursor 语义；当前后端任务是
`S4-002` SQLite 搜索实现。搜索尚未 Backend Ready，Stage 5 仍未授权。
