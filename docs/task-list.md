...
一句话：**认证、媒体库、扫描、浏览与缩略图后端已经 Backend Ready；`S4-002` 搜索实现
已完成，后端下一项是 `S4-003` 搜索容量与 Backend Gate；前端仍不能接入搜索。**
...
| 4. 搜索和查看器 | 搜索、筛选、查看大图和播放兼容视频 | 🟡 搜索后端已实现，尚未 Backend Ready | MVP 的主要使用流程完整 |
...
`S4-002` 已实现 migration 10、FTS/短词精确查询、三种 scope、筛选/排序、库内 generation
与跨库 revision cursor、离线和认证 HTTP；当前后端任务是 `S4-003` 容量/并发与 Backend
Ready Gate。搜索仍未交接前端，Stage 5 仍未授权。
