...
一句话：**方案、架构和高风险技术验证已经完成；正式后端入口已经建立，应用组装尚未完成。**

目前还没有可供用户使用的 FolioPath。下一步是完成唯一应用组合根，再让服务连接数据库、
提供健康检查并安全退出，之后实现管理员登录。

| 阶段 | 用普通话解释 | 状态 | 完成后能看到什么 |
| --- | --- | --- | --- |
| 0. 开工准备 | 定需求、定架构、验证 Docker/SQLite/媒体处理是否可行 | ✅ 已完成 | 确认方案能做，风险有人负责 |
| 1. 程序骨架和登录 | 建正式后端、数据库启动流程和管理员登录 | 🟡 当前阶段 | 服务可启动，可以初始化管理员并登录 |
| 2. 媒体库和扫描 | 在设置中选择 `/library` 子目录，创建多个媒体库并扫描 | ⬜ 未开始 | 可以创建媒体库并看到扫描状态 |
| 3. 文件夹浏览和缩略图 | 展示目录树、图片/视频缩略图和递归浏览 | ⬜ 未开始 | 可以按真实文件夹浏览媒体 |
| 4. 搜索和查看器 | 搜索、筛选、查看大图和播放兼容视频 | ⬜ 未开始 | MVP 的主要使用流程完整 |
| 5. 发布 | 安全、恢复、性能、双架构镜像和部署文档验收 | ⬜ 未开始 | 才可以称为可发布版本 |

### 当前只做什么

`S1-001` 已完成。当前任务是 **S1-002：创建 `internal/app` 唯一组合根**：
...
在登录后端完成前，不做媒体库 UI；在媒体库和扫描后端完成前，不做浏览页面。这样可以避免
先画出很多页面，最后因为后端模型变化而返工。
...
- [x] `S1-001` 创建最小 `cmd/foliopath`，只负责参数解析、启动和退出码。
  - 完成证据：`cmd/foliopath` 支持默认/`serve` 委托、`version`、help、usage/失败退出码；
    只导入 `internal/app`，测试验证 argv/env 交接且启动错误不泄露内部细节。
