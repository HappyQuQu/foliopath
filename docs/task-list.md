...
一句话：**认证、媒体库、扫描、浏览、缩略图与搜索后端已经 Backend Ready；认证前端已
Integrated Done，媒体库与扫描前端已完成主要真实功能，正在关闭 Stage 2 验收矩阵。**
...
| 2. 媒体库和扫描 | 在设置中选择 `/library` 子目录，创建多个媒体库并扫描 | 🟡 前后端已集成，验收中 | 可以创建、改名、扫描、取消和安全移除媒体库 |
...
- 当前：完整静态原型已作为设计基线确认；生产前端 Stage 1 已完成应用壳、主题/语言、
  ...
  Stage 2 已完成真实媒体库列表、三步创建、改名、异步安全移除、扫描详情/取消/重试和
  扫描周期/缓存配额设置；下一步关闭 `S2-207～208` 状态矩阵与 Integrated Done Gate。
