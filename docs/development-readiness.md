...
三种 scope、类型/mtime 筛选、排序和 cursor 语义；当前后端进入 `S4-002` SQLite 搜索实现。
这不表示搜索 handler 或搜索前端已经可用。
...
| 数据 | 首个 schema、迁移工具、外键/索引、generation 与任务恢复测试方案 | Goose migration 1～9 已执行；媒体库 revision/removal/idempotency、扫描与媒体 durable contract、typed settings、目录/资产自然名称 keyset 派生键、asset source fingerprint、thumbnail 派生状态与 cache deletion 具备真实升级与约束测试；发布版本升级仍待 Release Gate |
