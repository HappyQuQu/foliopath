...
`UIF-401～408` 逐页比较、真实纵向集成和发布前复验；其中 `UIF-401～403` 已完成，
当前进入 `UIF-404` 浏览器与可访问性复验。账户维护、直接子目录 `q`
...
| 测试 | 测试层次、合成 fixture、风险用例和发布门槛 | 原生双架构 Go/race、Web 契约、媒体、mount、runtime/recovery 与 Stage 0 SBOM CI 已通过；候选镜像/Compose 同入口已在本机原生 arm64 与指定原生 amd64 服务器通过。S5-005 的真实候选 100k/10k、100k 全量媒体、cache 水位和三引擎 100k FPS/RSS 预算已通过；S5-006A 的 Firefox/WebKit 稳定状态与 Linux 视觉基线已本机通过，Safari 26.5.2 真机纵向链和 Chrome 150 normal/forced-colors 通过；S5-007G 的修复来源 GLib 将本机 arm64 候选扫描降至 `0 Critical / 0 High`。真实 Firefox、读屏/缩放/触摸与移动物理客户端、最终原生双架构供应链复扫及正式安全/合规签署仍阻断 |
