...
| 媒体格式 | 确认 JPEG/PNG/WebP/GIF 图片与 MP4/MOV/MKV 视频；相同 govips/FFmpeg fixture 已在原生 amd64/arm64 验证元数据、缩略图、动画首帧策略和截断输入拒绝 | ICC/更多敌意输入、浏览器直放和生产任务隔离尚无实测 | 在媒体 Backend/UI Gate 验证超时、资源上限、取消、浏览器和最终镜像 | FS-03 Stage 0 范围满足；完整矩阵仍 Conditional |
