...
| 扫描与性能 | generation、流式遍历、短事务批量写入和故障保留已有自动化正确性证据；FS-04 在受限 Linux/tmpfs 上完成 10 万媒体/1 万目录、最大深度 32 的混合扫描/索引档、1,000 层 SQLite rollup、Linux RSS 与三档趋势 | 代表性 NAS 存储、媒体探测/缩略图、FTS、正式 HTTP、全局队列和前端交互仍无完整实测 | 按 S0-106 分配在对应 Backend/UI/Release Gate 测量生产能力，在代表性设备冻结发布预算 | Stage 0 扫描/索引可行性范围通过；叶到根批量 rollup 已去掉 O(D×A) 和 O(A×depth) 路径并对损坏拓扑失败关闭，完整 FS-04 仍 Conditional |
| 媒体格式 | 确认 JPEG/PNG/WebP/GIF 图片与 MP4/MOV/MKV 视频；S3-004 已将 govips/FFmpeg 变成 production adapter；S3-005 已验证 durable 2-worker/3-attempt 任务、fingerprint 失效、原子发布及缓存水位/余量 | ICC/更多敌意输入、原生调用不可中断区间、像素/磁盘满/资源峰值、浏览器直放和发布镜像尚无完整实测 | S3-006 验证敌意输入/资源/磁盘/取消，媒体 UI 与 Release Gate 验证浏览器和最终镜像 | FS-03 Stage 0、S3-004/005 实现范围满足；完整矩阵仍 Conditional |
| SQLite | 真实文件数据库、Goose 迁移、WAL、generation、原子 finalize、离线备份/恢复、重复 migration 与满盘/损坏失败关闭已验证 | 目标规模 FTS5/并发、真实强杀、长期 checkpoint、在线备份和真实跨版本升级未知 | 对应 Backend/Release Gate 继续故障与升级演练 | FS-02/05 Stage 0 范围通过；`/app/data` 仍禁止放在 SMB/NFS |
...
| [FS-04 容量基线](spikes/fs-04-capacity-baseline.md) | **Passed（Stage 0 范围）；完整范围 Conditional** | Linux/arm64、4 核/4 GiB、10 万媒体/1 万目录混合宽度/32 层深度；另有 1,000 层 finalize、Linux RSS、三档趋势与暂定回归预算 | 代表性磁盘/最终镜像、生产媒体队列、FTS/keyset、HTTP/前端并发按 [S0-106](gates/MVP-2026-07-23/s0-106-capacity-gate-order.md) 由后续 Gate 强制 |
