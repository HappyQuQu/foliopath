...
| 扫描与性能 | generation、流式遍历、短事务批量写入和故障保留已有自动化正确性证据；FS-04/S2-106 在受限 Linux/tmpfs 上完成 10 万媒体/1 万目录和 1,000 层 rollup；S4-003 又以同一主档验证扫描并发搜索、FTS/短词/全局/keyset、取消、rebuild、RSS 与 DB 预算 | 代表性 NAS 存储、媒体探测/缩略图组合容量、正式 HTTP/前端交互和发布镜像仍无完整实测 | 按 S0-106 继续在对应 UI/Release Gate 测量剩余生产能力，在代表性设备冻结发布预算 | 扫描与搜索后端目标档通过；完整 FS-04 仍 Conditional |
