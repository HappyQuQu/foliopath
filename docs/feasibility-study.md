...
| 媒体格式 | 确认 JPEG/PNG/WebP/GIF 图片与 MP4/MOV/MKV 视频；S3-004 已将 govips/FFmpeg 变成 production adapter，并验证元数据、缩略图/首帧、截断输入、超时分类、派生键与原子发布 | ICC/更多敌意输入、原生调用不可中断区间、浏览器直放、durable 任务隔离和发布镜像尚无完整实测 | S3-005/006 验证任务/资源/磁盘/取消，媒体 UI 与 Release Gate 验证浏览器和最终镜像 | FS-03 Stage 0 范围满足；S3-004 处理基础通过，完整矩阵仍 Conditional |
