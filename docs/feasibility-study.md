...
| 扫描与性能 | generation、流式遍历、短事务批量写入和故障保留已有自动化正确性证据；FS-04 在受限 Linux/tmpfs 上完成 10 万媒体/1 万目录、最大深度 32 的混合扫描/索引档、1,000 层 SQLite rollup、Linux RSS 与三档趋势；S4-002 已实现并验证 trigram FTS、短词 fallback 与 keyset 正确性 | 代表性 NAS 存储、搜索 100k/扫描并发预算、媒体探测/缩略图、正式 HTTP 容量、全局队列和前端交互仍无完整实测 | 按 S0-106 分配在对应 Backend/UI/Release Gate 测量生产能力，在代表性设备冻结发布预算 | Stage 0 扫描/索引可行性范围通过；搜索正确性已落地但容量仍等待 S4-003，完整 FS-04 仍 Conditional |
...
| SQLite | 真实文件数据库、Goose migration 1～10、WAL、generation、原子 finalize、FTS5/规范搜索键旧库回填、离线备份/恢复、重复 migration 与满盘/损坏失败关闭已验证 | 目标规模 FTS5/扫描并发、FTS rebuild/integrity 故障、真实强杀、长期 checkpoint、在线备份和真实已发布版本升级未知 | S4-003 与 Release Gate 继续容量、故障和升级演练 | FS-02/05 Stage 0 范围通过；S4-002 搜索正确性通过；`/app/data` 仍禁止放在 SMB/NFS |
...
| [FS-04 容量基线](spikes/fs-04-capacity-baseline.md) | **Passed（Stage 0 范围）；完整范围 Conditional** | Linux/arm64、4 核/4 GiB、10 万媒体/1 万目录混合宽度/32 层深度；另有 1,000 层 finalize、Linux RSS、三档趋势与暂定回归预算；S4-002 已补搜索实现与小型正确性矩阵 | 代表性磁盘/最终镜像、生产媒体队列、FTS/短词/keyset 100k、HTTP/前端并发按 [S0-106](gates/MVP-2026-07-23/s0-106-capacity-gate-order.md) 由 S4-003 和后续 Gate 强制 |
