...
| AF-011 | 大列表只使用游标分页和统一虚拟化模式 | API 契约测试、SQLite query 测试、前端 pattern 测试、E2E DOM/请求上限 | **后端查询已执行**：目录/资产默认 50、最大 200，query/generation-bound opaque keyset、完整 tuple、唯一 ID tie-breaker、context cancellation 与生产 SQLite 禁用 `OFFSET` 已由 S3-001/002 固定；S4-002 进一步固定库内 generation 与跨库 catalog-revision search cursor、global name tuple 和 mtime tuple。前端统一虚拟化和 E2E DOM/请求上限仍待对应前端 Gate | 浏览/搜索切片持续强制 |
