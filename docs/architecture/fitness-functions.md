...
| AF-007 | 失败、取消、离线或中断扫描绝不清理旧索引 | generation 故障矩阵、重启与竞态测试 | **部分自动化**：FS-02 与 S2-103 已验证失败/取消/offline/根替换不发布新目录计数或清理旧代次，成功 finalize 才原子发布；强杀、权限与磁盘故障仍待 S2-105/Release Gate | 持续，发布前补强杀/磁盘故障 |
