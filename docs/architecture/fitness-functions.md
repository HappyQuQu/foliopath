...
- **本地已执行**：仓库已有可运行入口，并在当前环境实际通过；当前不配置自动 CI，
  每次合并和发布由开发者负责执行对应本地入口。
...
| --- | --- | --- | --- | --- |
| AF-001 | Go 依赖只向内，handler 不依赖 SQLite/files，能力包不依赖 adapter/app | `make arch-check` 解析实际 Go import graph | **本地执行**；历史原生 amd64/arm64 结果保留为平台证据 | 持续强制 |
| AF-002 | 不出现无所有权的 `utils/common/helpers/base` 或无外部消费者的 `pkg/` | `make arch-check` 检查 Go package | **本地执行**；历史原生 amd64/arm64 结果保留为平台证据 | 持续强制 |
...
| AF-009 | 前端依赖方向、共享组件唯一所有权与 token 单一来源 | TypeScript boundary/cycle lint、受限基础库/import 位置 allowlist、token lint、组件目录检查 | **本地入口已建立**：strict TypeScript、唯一生成 client、禁止散落 `fetch`、app/routes/features/components/lib 依赖方向、共享组件目录、CSS token 与颜色字面量由 `npm run check:architecture` 强制；认证首个业务 feature 通过该门禁 | 持续强制 |
| AF-010 | 前端稳定原语在主题、语言、宽度和异步状态下行为一致 | component workbench build、Testing Library、axe、聚焦视觉回归 | **S5-006A 候选自动化已建立**：既有 Storybook/Testing Library、简中/英文、浅/深主题、390/768/1024/1440 与 Chromium axe 继续执行；Firefox/WebKit 新增查看器键盘、焦点、降级状态、overflow 与 axe 矩阵，固定 Linux Chromium 深色/reduced-motion 视觉基线已独立复跑；Safari 26.5.2、Chrome 原生 200% 和 Firefox 153.0.1 核心链/原生 200%/400% 已通过。物理读屏/触控/移动设备、Safari 缩放和最终视觉签署仍待 S5-006B | 行为/axe 在首次消费前；视觉基线在 API 稳定或第二消费者前；完整矩阵在 RC 前 |
...
| AF-012 | 单容器、root runtime、`/library:ro`、本地 root-owned `/app/data` 运行 | 双架构容器 smoke、安全挂载和健康检查 | **重新打开**：[ADR-0012](../adr/0012-root-runtime-bind-data.md)以零初始化 bind 数据目录为目标替代固定 UID/GID 65532；既有非 root 双架构证据保留为历史，但新的 root 候选必须复验数据库启动、只读媒体、cap-drop、health、SIGTERM 与 Compose | 首个预览镜像前 |
...
| AF-017 | SQLite 查询以 adapter 内的 SQL 源为唯一事实，生成代码不可手改或漂移 | sqlc 固定版本、临时目录重生成 diff、生成标记与 adapter 重复 SQL 检查 | **本地执行**：媒体库及 scan claim/lease/recovery 查询由 `queries/` 生成到 `dbgen/` 并被 adapter 消费；复杂 generation finalize 仍在 adapter 内受事务测试约束 | 持续强制 |
...
- 日常改动：格式、生成漂移、AF-001/002、静态检查、单元和受影响集成／组件测试。
- 合并前：全量 race、契约、E2E、组件工作台和聚焦视觉回归。
- 候选版本：在目标机器本地执行双架构镜像、目标容量、安全扫描、强杀／磁盘故障、
  备份恢复和升级演练。
