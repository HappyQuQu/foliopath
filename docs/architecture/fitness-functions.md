...
| AF-007 | 失败、取消、离线或中断扫描绝不清理旧索引 | generation 故障矩阵、重启与竞态测试 | **扫描故障矩阵已自动化**：FS-02、S2-103～105 已验证失败、取消、offline、部分不可读、nested mount、根替换和重启不发布不可靠代次或清理旧索引；startup scan 与晚到期 lease 通过真实 composition 自动恢复并在完整成功后收敛。磁盘满、数据库 I/O 与生产强杀仍由 S2-106/Release Gate 补齐 | 持续，发布前补强杀/磁盘故障 |
| AF-008 | 后台任务、数据库写入和媒体工具全部有界 | 队列/并发配置测试、压力指标、取消与超时测试 | **部分自动化**：S2-102/105 已固定容量 1 wake signal、2 个全局 scan worker、256 catalog batch、heartbeat/lease/cancel、周期过期恢复，以及按 ID/64 条分页的 startup admission；架构测试固定唯一 owner。媒体工具与 S2-106 压力指标待后续 Gate | 对应 worker 持续强制；容量证据在 S2-106 |
