...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `FR-DEP-001～004` 部署与设置 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/app`、`internal/api`、`internal/webassets`、`internal/store/sqlite`；容器适配器 | 启动/初始化、`/health/live`、`/health/ready`、`/api/v1/status`、设置读写 | migrations、`settings`、运行版本与健康状态；Vite 产物为构建输出 | [ADR-0001](../adr/0001-go-react-sqlite.md)、[ADR-0009](../adr/0009-linux-openat2-single-media-root.md)、[架构](../architecture.md)、[部署](../deployment.md) | R-002、R-004、R-008、R-011、R-014、R-016 | liveness、readiness 安全状态/停机转换、受保护 status 与能力格式已有单元测试；仍需空数据启动、迁移后 ready、单一只读媒体根且无后代挂载、非 root 容器、双架构 smoke、备份/恢复/升级 | 1、5 |
| `FR-AUTH-001～004` 认证与会话 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/auth` 拥有规则；`internal/api` 与 SQLite adapter | 首次 setup、login、session、logout；受保护业务 API 与 CSRF 失败流 | `users`、`sessions`；密码/会话秘密不进入日志 | [ADR-0005](../adr/0005-built-in-single-admin-auth.md)、[安全模型](../security.md) | R-010、R-011、R-016 | 并发初始化只能成功一次、密码与限流、会话过期/撤销、logout、CSRF、Cookie/代理、未认证媒体拒绝 | 1（功能性认证 S4）、5（发布加固） |
| `FR-LIB-001～008` 媒体库管理 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/library`；`internal/files` 和 SQLite adapter | 允许目录选择、媒体库创建/列表/详情/改名/移除、离线重试 | `libraries`；所有关联记录以 `library_id` 作用域；根路径创建后不可变 | [ADR-0002](../adr/0002-library-path-model.md)、[ADR-0004](../adr/0004-library-root-immutable.md)、[ADR-0009](../adr/0009-linux-openat2-single-media-root.md)、[安全模型](../security.md) | R-002、R-003、R-012、R-016 | [FS-01](../spikes/fs-01-path-boundary.md) 已有 Darwin 与原生 Linux amd64/arm64 路径、openat2 mount 与 HTTP harness 子范围；仍需生产 handler/认证错误边界、只读发布 volume、运行期 unmount，以及重叠根、唯一名称、根不可改、离线/根替换、幂等创建、移除后原文件逐字节不变 | 2 |
| `FR-SCN-001～009` 扫描与索引 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/scanner`、`internal/jobs`；`internal/files` 与 SQLite adapter | 创建/启动/定时/手动扫描、历史/详情轮询、协作取消、错误和跳过统计 | `scan_runs`、`directories`、`assets`、`media_jobs`、generation、聚合计数、扫描设置 | [ADR-0003](../adr/0003-scan-consistency.md)、[数据模型](../data-model.md) | R-003、R-004、R-005、R-013、R-016 | 分批提交、失败/取消/中断/离线不清理、成功 finalize 原子收敛、同库并发、跨库隔离、重启、磁盘/I/O/WAL 故障 | 2；容量证据在 0/FS-04 |
| `FR-BRW-001～009` 导航与浏览 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/catalog`；API 查询 adapter；`web` browse feature | 媒体库切换、目录树/面包屑、当前/递归浏览、布局与排序、URL 恢复 | `directories`、`assets`、`natural_name_key`、直接/递归计数；客户端只保留 URL/显示偏好 | [ADR-0001](../adr/0001-go-react-sqlite.md)、[界面设计](../ui-design.md) | R-005、R-012、R-013、R-015、R-016 | 空目录/深目录、稳定 keyset cursor、并发扫描下无重复漏项、刷新/返回恢复、虚拟化 DOM 上限、键盘/焦点 | 3 |
...
| `FR-MED-001～008` 缩略图、查看器与视频 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/media`、`internal/thumbnail`、`internal/jobs`；`internal/files`、govips、FFmpeg adapter；`web` viewer | 资产详情、thumbnail、原内容/Range；图片查看器、原生视频、不兼容/损坏状态、缓存设置 | `assets`、`thumbnails`、`media_jobs`、source fingerprint；缓存文件在 `/app/data/cache` | [ADR-0001](../adr/0001-go-react-sqlite.md)、[安全模型](../security.md)、[数据模型](../data-model.md) | R-006、R-007、R-008、R-009、R-014、R-016 | [FS-01](../spikes/fs-01-path-boundary.md) 已有测试 HTTP Range/HEAD/条件请求，[FS-03](../spikes/fs-03-media-matrix.md) 已有局部格式矩阵；仍需生产 handler/认证、魔数/损坏/炸弹/超时、取消、缓存失效/LRU/磁盘余量、浏览器与双架构行为 | 0/FS-01/03；3（缩略图/缓存）、4（查看器/视频） |
...
| `NFR-SEC-001～002` 路径与网络安全；`NFR-PRIV-001` 信息披露 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/files`、`internal/auth`、API middleware、错误/日志适配器、反向代理信任配置 | 逃逸与后代 mount crossing 失败关闭；无有效会话不返回业务数据；错误不泄露路径、SQL、stderr、Cookie 或令牌 | ADR-0002、ADR-0005、ADR-0009；R-002、R-010、R-012、R-016 | 服务端 request ID、统一安全 404/500、panic/HTTP runtime 日志脱敏已有单元测试；[FS-01](../spikes/fs-01-path-boundary.md) 已验证原生 Linux amd64/arm64 openat2 mount 与 HTTP harness；生产业务 handler/auth 由 Backend Gate 强制，发布 volume/unmount 由 FS-05/Release Gate 强制；仍需认证/CSRF/限流、代理和依赖安全测试 | 0、1、2、5 |
| `NFR-REL-001` 扫描与恢复一致性 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | scanner generation 状态机、SQLite 短事务、restart-safe jobs、原子缓存落盘 | 只有完整成功扫描可清理；崩溃、取消、离线和部分错误保留可靠状态并可收敛 | ADR-0003；R-003、R-004、R-011、R-013 | FS-02 generation 故障矩阵与 FS-05 离线恢复/满盘/损坏失败关闭已通过；正式应用强杀、真实升级和在线备份仍待 Gate | 0、2、5 |
...
| `NFR-OPS-001` 可运维性 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/app` 配置/生命周期、SQLite WAL、health、日志/指标、migration、备份恢复、缓存清理 | 本地可靠文件系统；安全启动/退出/升级/恢复；缓存和磁盘问题不能破坏配置或原件 | ADR-0001；R-004、R-009、R-011、R-016 | `internal/app`/`internal/api` 单元测试已覆盖固定根与安全监听、JSON 日志、health/readiness/status、顺序启动/失败回滚、HTTP 在途请求排空、反向关闭和停机期限；FS-05 probe 已验证 PID 1 退出、离线恢复、重复 migration 和故障关闭；正式数据库/migration ready 接线、在线备份与真实升级仍待 Gate | 1、5 |
