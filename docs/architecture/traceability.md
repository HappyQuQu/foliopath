...
| `FR-BRW-001～009` 导航与浏览 | `MVP-2026-07-23` | Frozen r1；[S3-001 Contract Ready](../gates/MVP-2026-07-23/s3-browse-contract-ready.md) | `BASELINE-2026-07-23` | `internal/catalog` 拥有 root/目录/资产 query normalization、排序与 cursor payload；`internal/cursor` 提供 token 机制；SQLite/API adapter；`web` browse feature | 媒体库切换、目录树/面包屑、当前/递归浏览、布局与排序、URL 恢复 | `directories`、`assets`、`libraries.current_generation`、计划追加的 `natural_name_key`/浏览索引、直接/递归计数；客户端只保留 URL/显示偏好 | [ADR-0001](../adr/0001-go-react-sqlite.md)、[界面设计](../ui-design.md)、[S3 浏览契约](../api-design.md#s3-浏览契约) | R-005、R-012、R-013、R-015、R-016 | S3-001 已固定可序列化 root、1～2049 项 root-to-current breadcrumb、direct/recursive scope、两个 keyset tuple、generation cursor、跨库 404、offline preserved index 和请求不访问文件系统；S3-002～007 仍需追加 migration、生产查询/HTTP、空/深/损坏拓扑、容量和 Backend Ready | 3（S3-001 完成；当前 S3-002） |
