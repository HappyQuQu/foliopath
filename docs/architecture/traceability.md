...
| `FR-AUTH-001～004` 认证与会话 | `MVP-2026-07-23` | Frozen r1；[Contract Ready](../gates/MVP-2026-07-23/s1-auth-contract-ready.md) | `BASELINE-2026-07-23` | `internal/auth` 拥有规则；`internal/api` 与 SQLite adapter | 首次 setup、login、session、logout；受保护业务 API 与 CSRF 失败流 | `users` singleton、password verifier、`sessions` 摘要/期限/撤销/auth version；秘密不进入数据库明文、响应或日志 | [ADR-0005](../adr/0005-built-in-single-admin-auth.md)、[安全模型](../security.md) | R-010、R-011、R-016 | OpenAPI 已固定 Origin、Cookie/CSRF、防缓存与错误码。S1-102～104 已实现密码、原子 setup、摘要化绝对期限会话、五个 handler、默认拒绝、Origin/CSRF、防缓存、直连 peer 限流和跨重启真实 HTTP 流；S1-105 已完成五类错误脱敏、凭据失败一致性、过期边界、真实并发 setup 单胜者与并发 session/SQLite 测试。仍需 S1-106 Backend Ready，以及发布阶段可信代理和浏览器 E2E | 1（S1-101 Contract Ready；S1-102～105 已实现并验收；S1-106 Backend Ready）、5（发布加固） |
| `FR-LIB-001～008` 媒体库管理 | `MVP-2026-07-23` | Frozen r1 | `BASELINE-2026-07-23` | `internal/library`；`internal/files` 和 SQLite adapter | 允许目录选择、媒体库创建/列表/详情/改名/移除、离线重试 | `libraries`；所有关联记录以 `library_id` 作用域；根路径创建后不可变；SQL 由 adapter 内 sqlc query 源生成 | [ADR-0001](../adr/0001-go-react-sqlite.md)、[ADR-0002](../adr/0002-library-path-model.md)、[ADR-0004](../adr/0004-library-root-immutable.md)、[ADR-0009](../adr/0009-linux-openat2-single-media-root.md)、[安全模型](../security.md) | R-002、R-003、R-012、R-016 | 媒体库 SQL 生成漂移、生成标记、adapter 唯一消费边界与既有 service/store 测试已执行；[FS-01](../spikes/fs-01-path-boundary.md) 已有路径边界证据；仍需生产 handler/认证错误边界、只读发布 volume、运行期 unmount，以及离线/根替换、幂等创建、移除后原文件逐字节不变 | 2 |
