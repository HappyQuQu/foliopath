...
| `FR-SRH-001～004` 搜索过滤排序 | `MVP-2026-07-23` | Frozen r1；[S4-001 Contract Ready](../gates/MVP-2026-07-23/s4-search-contract-ready.md) | `BASELINE-2026-07-23` | `internal/catalog` 拥有 search profile、scope、排序与 cursor；SQLite FTS adapter；`web` search feature | 当前目录（可递归）/当前库/全部库搜索，类型和 filesystem mtime 半开区间过滤，结果查看/返回 | `assets` 与计划的 FTS5/规范搜索键、`catalog_search_state.revision`；库内 generation 与跨库 revision cursor | [ADR-0001](../adr/0001-go-react-sqlite.md)、[数据模型](../data-model.md)、[S4 搜索契约](../api-design.md#s4-搜索契约) | R-005、R-012、R-016 | S4-001 已固定 profile v1 的 NFKC/full case folding、字面子串 AND、短词/标点、三种 scope、kind/mtime、稳定 name/mtime tuple、offline 与 cursor 失效；S4-002 仍须实现 migration/FTS/keyset 并以中文/英文/组合字符/短查询、取消、重建和 100k 容量 fixture 验证，S4-003 后才可 Backend Ready | 4（Contract Ready；当前 S4-002） |
