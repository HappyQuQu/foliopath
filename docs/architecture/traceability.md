...
| `FR-UI-001～007` 界面与可访问性 | `MVP-2026-07-23` | Frozen r1；[S4-005 搜索预览 Done](../gates/MVP-2026-07-23/s4-frontend-search-preview.md) | [CR-2026-001](../changes/CR-2026-001-non-modal-media-preview.md) | `web` 的 auth/libraries/browse/search/viewer/settings feature；统一 API client；共享 `MediaCollection`、`MediaPreview` 与 `useMediaPreviewController` | 欢迎/认证、设置、扫描状态、浏览/搜索/预览/查看器；桌面侧栏、移动抽屉、中英与主题 | 服务端状态不复制为独立事实；URL 保存导航状态，`settings` 保存受控偏好；固定预览保存当前媒体快照并跨搜索结果刷新继续存在 | [ADR-0001](../adr/0001-go-react-sqlite.md)、[用户流程](../user-flows.md)、[界面设计](../ui-design.md) | R-005、R-010、R-015、R-016 | loading/empty/error/offline/cancel、预览选择/固定/双击、搜索刷新不中断固定媒体、键盘/焦点/读屏、主题/语言/reduced-motion、响应式、虚拟化、关键 E2E | 1（认证 UI）、3；4（S4-004～005 Done，完整查看器待完成）；5（发布加固） |
