...
现冻结 A～E。INT-S0/INT-S1 仍只对 A+B 为 Go，INT-S2A 已复审为 No-Go；C/D/E 的 INT-S1R2 为
Go，授权 S2B 后端实现；S2C 仍由独立隐私/合法数据/候选许可准入保持 No-Go。
S2A/S2B Backend Gate 均为 No-Go，且 S2C 尚未准入，三者都不授权生产 UI。
对应长表行继续保留完整历史证据，不再追加同类合成测试。
...
| `FR-INT-001～020`、`NFR-INT-001～010` 本地智能媒体发现 | `POST-MVP-5` r2 | [Frozen revision 2](../releases/POST-MVP-5-scope-r2.md)，A～E；A+B [INT-S2A](../gates/POST-MVP-5/int-s2a-backend-evidence-ready.md) No-Go；C/D/E [INT-S1R2](../gates/POST-MVP-5/int-s1r2-contract-ready.md) Go；[S2B](../gates/POST-MVP-5/int-s2b-backend-evidence-ready.md) 已授权/No-Go；[S2C privacy](../gates/POST-MVP-5/int-s2c-privacy-ready.md) No-Go | [CR-2026-021](../changes/CR-2026-021-intelligent-media-discovery.md) | A+B/C/D：`internal/semantic`、`internal/aimodel` 与 curation/media/thumbnail 窄 ports；E：后续 `internal/face` 唯一 owner；SQLite/API/app adapters | 模型基础、图片语义搜索、受控标签建议、完整故事板视频语义搜索、匿名人脸聚类与用户人物库；仍禁止身份推断、自动标签、云/GPU/新部署单元 | A+B migration 22/23/24 与后端边界已存在但 Gate No-Go；C/D migration 25～32、service/repository/worker/HTTP 边界已实现，标签审核清理和视频 job 已 production composition；tag generation/video search 仍因未接受 text runtime 不注册；S2C 禁止 composition | [revision 2 scope](../releases/POST-MVP-5-scope-r2.md)；[A+B S1 contract](intelligent-media-s1-contract.md)；[C/D/E capability](intelligent-media-s1r2-contract.md)；`api/openapi.yaml` r2；data/security/deployment/testing contracts；ADR-0013/0014 | R-024～R-030 全部适用且开放 | A+B 仍缺 accepted tokenizer、最终模型/质量/双架构/容量/供应链；S2B 本地状态机已实现但缺合法 tag/video 质量、native amd64 与最终供应链；E 另缺隐私签署、合法 ground truth、≥99.5% core precision 和可分发 face 模型。S2C No-Go 下禁止 production face composition。 | `Frozen r2 A～E → S1R2 Go；S2B authorized/No-Go；S2A+S2C No-Go` |
