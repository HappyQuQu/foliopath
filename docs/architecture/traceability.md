...
| `FR-AUTH-001～004`、`FR-DEP-005` 认证与 LAN 部署 | `MVP-2026-07-23` | Frozen r2；认证 Integrated Done；LAN HTTP implemented | [CR-2026-002](../changes/CR-2026-002-authenticated-lan-http.md) | `internal/auth` 拥有规则；`internal/api` 拥有 transport/HTTP；`internal/app` 拥有监听配置；Compose 拥有宿主绑定 | 首次 setup、login、session、logout；受保护业务 API；受信 LAN HTTP 与可选严格 HTTPS 代理 | 认证数据不变；transport 不持久化，未受信转发头清除 | [ADR-0005](../adr/0005-built-in-single-admin-auth.md)、[ADR-0010](../adr/0010-authenticated-lan-http.md)、[安全模型](../security.md) | R-010、R-011、R-016 | 直连 HTTP 使用实际 peer/Host，保留认证/CSRF；显式可信代理配置继续严格校验 HTTPS 并拒绝旁路。Compose smoke 与 LAN 实机验证属于 Stage 5 证据 | 1（Integrated Done）、5 |
...
| `FR-MED-009～011`、`FR-UI-008` 视频故事板悬停预览 | `POST-MVP-1` | Frozen r1；[VSP-S1 Contract Ready](../gates/POST-MVP-1/vsp-s1-contract-ready.md) Go；后端进行中 | [CR-2026-004](../changes/CR-2026-004-video-storyboard-preview.md) | `internal/thumbnail` 拥有 variant/采样/派生/缓存；`internal/media` FFmpeg adapter；`internal/jobs` 调度；`internal/files` 安全打开；SQLite/cache/API adapter；共享 `MediaCollection` 和唯一 availability adapter | authenticated `thumbnail?variant=storyboard` 已冻结并实现；poster 后低优先级生成；桌面 fine-pointer hover 按需加载；touch/键盘/reduced-motion 回退 poster | migration 11 已实现 replacement tables、4/10 帧 layout CHECK、priority、source fingerprint/transform version、原子发布、统一 LRU 和 128 项 bounded admission；目标容量证据待 S2 | [FTR-VID-001](../features/video-storyboard-preview.md)；复用 ADR-0001/0006/0007/0009；若改变 job 一致性/所有权则先新增 ADR | R-006、R-009、R-013、R-015、R-016、R-018 | [VSP-002](../spikes/vsp-002-video-storyboard.md)验证 2s～2h fast seek/sprite；OpenAPI、生成 client、契约测试、S1 Gate、领域/SQLite/adapter/worker/API 单元与集成代码已通过；目标 Linux 双架构、容量 Backend Gate、共享 hover、axe/reduced-motion/虚拟化/三浏览器和真实纵向 E2E 仍待执行 | `VSP-S0 Done → S1 Done → 后端 → S2 → 前端 → S3 → S4` |
| `FR-UI-001～007` 界面与可访问性 | `MVP-2026-07-23` | Frozen r1；[S4-005 搜索预览 Done](../gates/MVP-2026-07-23/s4-frontend-search-preview.md)；[S4-006 Viewer Done](../gates/MVP-2026-07-23/s4-frontend-media-viewer.md)；[S4-007 Media Strategy Done](../gates/MVP-2026-07-23/s4-frontend-media-strategy.md)；[S4-008 Media Matrix Done](../gates/MVP-2026-07-23/s4-frontend-media-matrix.md)；[Stage 4 Integrated Done](../gates/MVP-2026-07-23/s4-search-media-integrated-done.md) | [CR-2026-001](../changes/CR-2026-001-non-modal-media-preview.md)；[CR-2026-003](../changes/CR-2026-003-brand-identity.md) | `web` 的 auth/libraries/browse/search/media/settings feature；统一 API client；共享 `BrandMark`、`MediaCollection`、`MediaPreview`、`MediaViewer`、`MediaAvailabilityState` 与 `useMediaPreviewController` | 欢迎/认证、设置、扫描状态、浏览/搜索/预览/查看器；统一品牌入口；桌面侧栏、移动抽屉、中英与主题 | 服务端状态不复制为独立事实；URL 保存导航状态，`settings` 保存受控偏好；固定预览保存当前媒体快照，查看器瞬时状态只保存已载入 ID 序列和安全返回位置；品牌 SVG 是无业务状态的静态资产 | [ADR-0001](../adr/0001-go-react-sqlite.md)、[ADR-0007](../adr/0007-shared-frontend-system.md)、[用户流程](../user-flows.md)、[界面设计](../ui-design.md)、[品牌标识](../branding.md) | R-005、R-010、R-015、R-016 | loading/empty/error/offline/cancel、预览选择/固定/双击、搜索刷新不中断固定媒体、查看器缩放/平移/信息/全屏/关闭恢复、媒体离线/缺失/不可读/损坏/不支持/删除、统一品牌 SVG/三个语义尺寸/装饰性语义、键盘/触摸/焦点/读屏、主题/语言/reduced-motion、响应式、虚拟化、关键 E2E | 1（认证 UI/品牌原语）、3、4（Integrated Done）；5（发布浏览器/真机加固） |
