...
| `FR-AUTH-001～004` 认证与会话 | `MVP-2026-07-23` | Frozen r1；[Contract Ready](../gates/MVP-2026-07-23/s1-auth-contract-ready.md)；[Backend Ready](../gates/MVP-2026-07-23/s1-auth-backend-ready.md)；[Integrated Done](../gates/MVP-2026-07-23/s1-auth-integrated-done.md)；[S5-003 代理安全](../gates/MVP-2026-07-23/s5-trusted-proxy-security.md) | `BASELINE-2026-07-23` | `internal/auth` 拥有规则；`internal/api` 拥有 transport/HTTP；SQLite adapter；`web/src/features/auth` 与 `web/src/lib/api/auth.ts` | 首次 setup、login、session、logout；受保护业务 API、可信代理与 CSRF 失败流 | `users` singleton、password verifier、`sessions` 摘要/期限/撤销/auth version；秘密不进入数据库明文、响应或日志；代理声明不持久化 | [ADR-0005](../adr/0005-built-in-single-admin-auth.md)、[安全模型](../security.md) | R-010、R-011、R-016 | 认证前后端已 Integrated Done；S5-003 增加显式可信 CIDR、非回环 require-proxy、严格单跳 HTTPS transport，并证明 Origin、Secure Cookie、HSTS 与限流客户端 key 共享同一验证结果。最终浏览器矩阵和实际发布网络拓扑仍由 S5-001B/002/006 阻断 | 1（Integrated Done）、5（代理加固完成） |
...
| `FR-UI-001～007` 界面与可访问性 | `MVP-2026-07-23` | Frozen r1；[S4-005 搜索预览 Done](../gates/MVP-2026-07-23/s4-frontend-search-preview.md)；[S4-006 Viewer Done](../gates/MVP-2026-07-23/s4-frontend-media-viewer.md)；[S4-007 Media Strategy Done](../gates/MVP-2026-07-23/s4-frontend-media-strategy.md)；[S4-008 Media Matrix Done](../gates/MVP-2026-07-23/s4-frontend-media-matrix.md)；[Stage 4 Integrated Done](../gates/MVP-2026-07-23/s4-search-media-integrated-done.md) | [CR-2026-001](../changes/CR-2026-001-non-modal-media-preview.md) | `web` 的 auth/libraries/browse/search/media/settings feature；统一 API client；共享 `MediaCollection`、`MediaPreview`、`MediaViewer`、`MediaAvailabilityState` 与 `useMediaPreviewController` | 欢迎/认证、设置、扫描状态、浏览/搜索/预览/查看器；桌面侧栏、移动抽屉、中英与主题 | 服务端状态不复制为独立事实；URL 保存导航状态，`settings` 保存受控偏好；固定预览保存当前媒体快照，查看器瞬时状态只保存已载入 ID 序列和安全返回位置 | [ADR-0001](../adr/0001-go-react-sqlite.md)、[用户流程](../user-flows.md)、[界面设计](../ui-design.md) | R-005、R-010、R-015、R-016 | loading/empty/error/offline/cancel、预览选择/固定/双击、搜索刷新不中断固定媒体、查看器缩放/平移/信息/全屏/关闭恢复、媒体离线/缺失/不可读/损坏/不支持/删除、键盘/触摸/焦点/读屏、主题/语言/reduced-motion、响应式、虚拟化、关键 E2E | 1（认证 UI）、3、4（Integrated Done）；5（发布浏览器/真机加固） |
...
| `NFR-SEC-001～002` 路径与网络安全；`NFR-PRIV-001` 信息披露 | `MVP-2026-07-23` | Frozen r1；[S5-003 代理安全](../gates/MVP-2026-07-23/s5-trusted-proxy-security.md) | `BASELINE-2026-07-23` | `internal/files`、`internal/auth`、API transport/middleware、错误/日志适配器、反向代理信任配置 | 逃逸与后代 mount crossing 失败关闭；无有效会话不返回业务数据；错误不泄露路径、SQL、stderr、Cookie 或令牌；未受信转发头不能改变安全判断 | ADR-0002、ADR-0005、ADR-0009；R-002、R-010、R-012、R-016 | request ID、统一脱敏错误、认证/CSRF/限流、openat2 mount/TOCTOU/ABA 与媒体读取边界已有测试；S5-003 已验证显式 CIDR、代理旁路拒绝、严格 HTTPS transport、Secure Cookie/Origin/HSTS/客户端限流和全局安全头。发布 volume/unmount 与实际 Compose 网络拓扑仍由 S5-001B/002/004 强制 | 0、1～4（Integrated Done）、5（代理加固完成） |
