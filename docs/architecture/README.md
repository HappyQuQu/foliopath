...
摘要锁、语义兼容检查和统一本地验证入口已建立；自动测试 CI 已关闭，历史双架构结果仍只
证明原记录的提交与环境。这些 scope 仍不能等同于产品完成。最小
...
| 智能媒体发现 A～E | [智能媒体发现技术架构](intelligent-media-discovery.md)、[S1/S1R2 capability/事务合同](intelligent-media-s1-contract.md)、[离线模型包合同](ai-model-package-contract.md) | revision 2 已冻结；S2A/S2B/S2C Backend Ready / Release No-Go，最终模型/质量/native/供应链归 S4 |
