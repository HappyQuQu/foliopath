...
**不适合：** 照片备份与手机同步、多人相册、现实身份识别、图片编辑或视频转码。

### 本地智能发现：已实现，尚未发布

`POST-MVP-5 revision 2` 已实现可选的本地图片/视频语义搜索、受控标签建议、匿名人脸分组和由管理员
命名的人物库。它**不会**识别现实身份、联网查人、训练模型、上传媒体或查询，也不会修改原文件。

在最终审核模型包、governed 质量结果、原生 `linux/amd64` 与 `linux/arm64` 配对证据、4 CPU/4 GiB
联合容量、供应链材料及所需批准通过 S4 Gate 前，这些能力保持发行禁用。FolioPath 不提供模型下载或
镜像入口；部署者只能自行取得 digest 与内建 reviewed catalog 精确匹配的包，并通过可选只读
`/models:ro` 挂载提供。模型缺失、变化、损坏、不兼容或未审核时，仅依赖它的智能能力不可用，普通
浏览继续。详见 [revision 2 冻结范围](docs/releases/POST-MVP-5-scope-r2.md)、
[部署合同](docs/deployment.md#post-mvp-5-revision-2-cde-部署与恢复合同)和
[安全与隐私合同](docs/security.md#post-mvp-5-revision-2-cde-安全与隐私合同)。
